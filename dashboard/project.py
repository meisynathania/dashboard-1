...
clean_day.groupby(by="holiday").agg({
    "instant": "nunique",
    "cnt": ["max", "min", "mean", "std"]
})
clean_hour.groupby(by="hr").agg({
    "cnt": ["max", "min", "mean", "std"],
})
...
seasonal = pd.DataFrame(clean_day.groupby(by=["season","yr"]).agg({
    "cnt": ["sum"],
}).unstack())
...
hourly_day = clean_hour[clean_hour["dteday"] == '2011-01-01']
...
if sidebarOpt == 'Daily Report' or sidebarOpt =='':
    ...
    hourly_day = clean_hour[clean_hour["dteday"] == str(dateInput)] #subset data per hari
    daily_day = clean_day[clean_day["dteday"] == str(dateInput)]
    ...
    selected_year = st.radio("Tahun", 
                             year, key="2011")
    subset_data = clean_day[(clean_day['yr'] == selected_year)]
